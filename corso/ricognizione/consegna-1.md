...
| Skill `/collaudo` («usala su ogni PR prima del merge») | CLAUDE.md, elenco delle skill | Mai invocata nell'intervallo: grep di `collaudo\|pr-fanout` nel journal → solo le due voci del 13/09 che dicono che non gira. |
| Workflow `Verifica agentica su PR` | `.github/workflows/claude-pr-review.yml` | `gh api …/actions/workflows` → `disabled_manually`. 2 run in tutta la storia, entrambi falliti il 13/09 su PR #1 e #2. Commenti di collaudo sulle 26 PR controllate: **0**. Gli unici commenti sono di `andreapernici`. |
